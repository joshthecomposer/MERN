...
const namesOfPoisonTypes = pokémon.filter(p => p.types.length == 1 && p.types.includes("poison")).map(p => p.name);
console.log(namesOfPoisonTypes);

const secondFlying = pokémon.filter(p => p.types[1] == "flying").map(p => p.types[0]);
console.log(secondFlying);

const justNormal = pokémon.filter(p => p.types.includes("normal")).length;
console.log(justNormal);